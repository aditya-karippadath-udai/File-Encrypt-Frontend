...
export interface DecryptionEventHandlers {
...
export const mockDecryptionEventBus = new MockDecryptionEventBus();

/**
 * Sets up listeners for native Tauri or mock browser decryption events.
 * Returns a cleanup unlisten function that unbinds all handlers.
 */
export function setupDecryptionEventListeners(handlers: DecryptionEventHandlers): () => void {
  ...
  if (isTauri()) {
    const attachTauriListeners = async () => {
      try {
        if (handlers.onBatchProgress) {
          const unlisten = await listen<BatchProgressPayload>('decryption://progress', (event) => {
        ...
        if (handlers.onJobProgress) {
          const unlisten = await listen<JobProgressPayload>('decryption://job-status', (event) => {
        ...
        console.warn('Failed to attach Tauri decryption event listeners:', err);
      }
    };
    ...
    if (handlers.onBatchProgress) {
      unlistenFns.push(mockDecryptionEventBus.onBatchProgress(handlers.onBatchProgress));
    }
    if (handlers.onJobProgress) {
      unlistenFns.push(mockDecryptionEventBus.onJobProgress(handlers.onJobProgress));
    }
    if (handlers.onOperationStatus) {
      unlistenFns.push(mockDecryptionEventBus.onOperationStatus(handlers.onOperationStatus));
    }
  }
  ...
  return () => {
    ...
    for (const unlisten of unlistenFns) {
      try {
        ...
        console.warn('Error during decryption event listener cleanup:', err);
      }
    }
  };
}
